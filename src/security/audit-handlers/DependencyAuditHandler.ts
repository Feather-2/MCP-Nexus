...
const RISKY_PACKAGE = /(^@?(evil|malicious|backdoor|hack|exploit))|(-stealer|-keylogger|-miner|-malware)$/i;
const VERSION_PIN = /@\d+\.\d+\.\d+/;
...
export class DependencyAuditHandler implements AuditSkillHandler {
  ...
  analyze(units: SemanticUnit[]): AuditSkillResult {
    ...
    const risky = imports.filter((target) => RISKY_PACKAGE.test(target));
    ...
    const unpinned = imports.filter((target) => isPackageImport(target) && !VERSION_PIN.test(target));
    ...
    if (imports.length > 20) findings.push({ auditSkill: this.name, severity: 'medium', message: 'Dependency import count is unusually high', evidence: `imports=${imports.length}`, unit: 'imports' });
