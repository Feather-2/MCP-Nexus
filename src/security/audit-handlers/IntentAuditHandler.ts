...
const DEFAULT_SUSPICIOUS_NAME_PATTERN = /\b(hack|exploit|reverse|shell|payload|backdoor)\b/i;
const DEFAULT_VAGUE_DESCRIPTION_PATTERN = /\b(todo|tbd|misc|various|something|generic)\b/i;
const DEFAULT_MAX_TOOL_COUNT = 10;
...
export interface IntentAuditHandlerOptions {
  suspiciousNamePattern?: RegExp;
  vagueDescriptionPattern?: RegExp;
  maxToolCount?: number;
}
...
export class IntentAuditHandler implements AuditSkillHandler {
  ...
  private readonly suspiciousNamePattern: RegExp;
  private readonly vagueDescriptionPattern: RegExp;
  private readonly maxToolCount: number;

  constructor(options: IntentAuditHandlerOptions = {}) {
    this.suspiciousNamePattern = options.suspiciousNamePattern ?? DEFAULT_SUSPICIOUS_NAME_PATTERN;
    this.vagueDescriptionPattern = options.vagueDescriptionPattern ?? DEFAULT_VAGUE_DESCRIPTION_PATTERN;
    this.maxToolCount = options.maxToolCount ?? DEFAULT_MAX_TOOL_COUNT;
  }
  ...
  analyze(units: SemanticUnit[]): AuditSkillResult {
    ...
    for (const unit of units) {
      ...
      if (this.suspiciousNamePattern.test(unit.content)) {
      ...
      if (compact.length < 24 || this.vagueDescriptionPattern.test(compact)) {
    ...
    if (declaredTools > this.maxToolCount) {
